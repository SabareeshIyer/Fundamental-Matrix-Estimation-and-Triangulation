function matches = matchImages (img_left, img_right)
...
%{
figure; imshow([img_left img_right]); hold on; 
...
putative_threshold = 4;
[left_matched_indices, right_matched_indices] = find(dist_mat < putative_threshold);
%putative_matches(X, C, dist_mat, putative_threshold);
matched_row_left  = row_left(left_matched_indices);
matched_col_left  = col_left(left_matched_indices);
matched_row_right = row_right(right_matched_indices);
matched_col_right = col_right(right_matched_indices);
...
%[homo, inliers] = homography(matchPointsLeft, matchPointsRight);
...
for i=1:numIter
    ...
    ssd = (sum(dif(:).^2));    
    inlier_thresh = 10;
    num_inliers = nnz(dif < inlier_thresh) - 4;
    ssd_store(i) =  sqrt(sqrt(ssd));
    if(num_inliers >= max_num_inliers && i ~= 1 && ssd < min_ssd)
           min_ssd = ssd;           max_num_inliers = num_inliers;           max_homo = homography;
           left_homo_pts = pointsForHomoLeft;           right_homo_pts = pointsForHomoRight;
    end
end
clear num_inliers;
H = max_homo;
max_num_inliers;
mean(ssd_store);
matches = [left_homo_pts(:,1:2), right_homo_pts(:,1:2)];
